...
| 平台       | 标识     | 自动操作                                     | 分类/标签              |
| ---------- | -------- | -------------------------------------------- | ---------------------- |
| 少数派     | `sspai`  | 填写标题 → 粘贴 Markdown → 触发格式转换      | —                      |
| 知乎       | `zhihu`  | 填写标题 → 粘贴 Markdown → 自动解析          | —                      |
| 微信公众号 | `wechat` | Markdown 转富文本 → 填写标题/摘要 → 粘贴内容 | —                      |
| 什么值得买 | `smzdm`  | 填写标题 → Markdown 转 HTML 后粘贴           | 自动选择分类、填写标签 |
| 掘金       | `juejin` | 填写标题 → 粘贴 Markdown 原文                | 自动选择分类、填写标签 |
...
- 一键检测各平台登录状态和用户名
- 关闭发布窗口释放资源（仅关闭自动化 Chrome，不影响本页面）
- 设置持久化（文章目录、默认平台等）
...
GUI 模式下可点击「检查登录」按钮并行检测所有平台的登录状态。
